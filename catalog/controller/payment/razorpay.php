...
class ControllerPaymentRazorpay extends Controller
{
    ...
    {
        /*THIS IS NOT WORKING. THIS CODE IS WRIITEN IN CONFIRM.PHP*/
    ...
    public function callback()
    {
        ...
        if (isset($this->request->request['razorpay_payment_id'])) {
            ...
            $razorpay_order_id = $this->request->request['razorpay_order_id'];
            ...
            $success = false;
            $error1 = 'Request Received';

            $error = NULL;

            $this->db->query("UPDATE ".DB_PREFIX."followup set payment_redirect = '".$error1."' WHERE order_id = '" . $merchant_order_id."'");
            ...
            $generated_signature  = hash_hmac('sha256', $merchant_order_id + "|" + $razorpay_payment_id, RAZORPAY_SECRET);
            //$generated_signature = hash_hmac('sha256', $order_id . '|' . $razorpay_payment_id, $secret);

            //echo $generated_signature.'---------------------------------------------'.$razorpay_signature; die;
