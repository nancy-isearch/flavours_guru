		...
		{ 
	        ...
	        $calculatedAmount_INR = $order_info['total'];  
				...
				if($this->config->get('payu_test')=='demo')
				{
					...
					$txnid        = 	$this->session->data['order_id'].'_'.uniqid();
				}
				else
				{
			...
			$salt         = $salt;
			if(isset($_SESSION['pType']['ccard_content'])){
				$data['ccNumber'] = $_SESSION['pType']['ccard_content']['ccNumber'];
				$data['ccname'] = $_SESSION['pType']['ccard_content']['ccname'];
				$data['ccexpmon'] = $_SESSION['pType']['ccard_content']['ccexpmon'];
				$data['ccexpyr'] = $_SESSION['pType']['ccard_content']['ccexpyr'];
				$data['ccCVV'] = $_SESSION['pType']['ccard_content']['ccCVV'];
				$data['pg'] = $_SESSION['pType']['ccard_content']['pg'];
				$data['cardname'] = $_SESSION['pType']['ccard_content']['cardname'];
				$data['bankcode'] = $_SESSION['pType']['ccard_content']['bankcode'];
			}
			elseif(isset($_SESSION['pType']['dcard_content'])){
				$data['ccNumber'] = $_SESSION['pType']['dcard_content']['ccNumber'];
				$data['ccname'] = $_SESSION['pType']['dcard_content']['ccname'];
				$data['ccexpmon'] = $_SESSION['pType']['dcard_content']['ccexpmon'];
				$data['ccexpyr'] = $_SESSION['pType']['dcard_content']['ccexpyr'];
				$data['ccCVV'] = $_SESSION['pType']['dcard_content']['ccCVV'];
				$data['pg'] = $_SESSION['pType']['dcard_content']['pg'];
				$data['cardname'] = $_SESSION['pType']['dcard_content']['cardname'];
				$data['bankcode'] = $_SESSION['pType']['dcard_content']['bankcode'];
			}
			elseif(isset($_SESSION['pType']['nbanking_content'])){
				$data['ccNumber'] = "";
				$data['ccname'] = "";
				$data['ccexpmon'] = "";
				$data['ccexpyr'] = "";
				$data['ccCVV'] = "";
				$data['pg'] = $_SESSION['pType']['nbanking_content']['pg'];
				$data['cardname'] = "";
				$data['bankcode'] = $_SESSION['pType']['nbanking_content']['bankcode'];
			}
		...
		if (isset($this->request->post['key']) && ($this->request->post['key'] == isset($key))) {
			...
			$data['text_failure_wait'] = sprintf($this->language->get('text_failure_wait'), $this->url->link('checkout/cart'));
			 ...
			 if($this->config->get('payu_test')=='demo')
				{
					...
					$orderid = explode("_",$this->request->post['txnid'])[0];
				} else 
				{
			 ...
			 $order_info = $this->model_checkout_order->getOrder($orderid);
				...
				//echo "<pre />"; print_r($_POST); print_r($order_info); die;

			 
			 if (isset($this->request->post['status']) && $this->request->post['status'] == 'success') {
			 	
			 	$saltString     = $keyString."||".$salt;
				...
				foreach($this->request->post as $k => $val){
					...
					if(true /*$sentHashString==$this->request->post['hash']*/){
			...
			if($this->request->post['unmappedstatus'] == 'initiated')
						...
						$sql2 = "UPDATE " . DB_PREFIX . "order SET custom_field = '" .$this->request->post['mihpayid'].",".$this->request->post['txnid'] ."' WHERE order_id= '" .$orderid . "'";
