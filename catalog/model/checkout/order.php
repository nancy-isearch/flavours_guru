...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		// print_r($data['products']);echo "upproduct";
		/*$shippingdateforadminfilter1 = '';
		if (isset($data['products'][0])){
			$shippingDate = $shippingdateforadminfilter = array();
				$shippingDate = json_decode($data['products'][0]['shipping_info'], true);
				$shippingdateforadminfilter = $shippingDate['Shipping Date'];
				$shippingdateforadminfilter1 = date("Y-m-d", strtotime($shippingdateforadminfilter));
				// echo $shippingdateforadminfilter1;die; 
		} else { $shippingdateforadminfilter1 = ''; }*/
		/*if (isset($data['products'][0])){
			$shippingDate = $shippingdateforadminfilter = array();
				$shippingDate = json_decode($data['products'][0]['shipping_info'], true);
				if(!empty($shippingDate)){
				  $shippingdateforadminfilter = $shippingDate['Shipping Date'];
				  $shippingdateforadminfilter1 = date("Y-m-d", strtotime($shippingdateforadminfilter));
				  // echo $shippingdateforadminfilter1.'->1';die; 
				} else {
				 $shippingDate = json_decode($data['products'][1]['shipping_info'], true);
				 // print_r($shippingDate);echo '->2';die;
				 $shippingdateforadminfilter = $shippingDate['Shipping Date'];
				 $shippingdateforadminfilter1 = date("Y-m-d", strtotime($shippingdateforadminfilter));
				}
		} else { $shippingdateforadminfilter1 = ''; }*/	
		...
		foreach($data['products'] as $key => $val){
			 print_r(json_decode($val['shipping_info'], true));
			$shipping_decode = $shippingdateforadminfilter2 = $shippingdateforadminfilter3 = array();
			$shipping_decode = json_decode($val['shipping_info'], true);
			$shippingdateforadminfilter2 = $shipping_decode['Shipping Date'];
			$shippingdateforadminfilter3 = date("Y-m-d", strtotime($shippingdateforadminfilter2));
			if(!empty(json_decode($val['shipping_info'], true)) && $shippingdateforadminfilter3 !== "000-00-00"){
				$shippingDate = $shippingdateforadminfilter = array();
				$shippingDate = json_decode($val['shipping_info'], true);
				$shippingdateforadminfilter = $shippingDate['Shipping Date'];
				$shippingdateforadminfilter1 = date("Y-m-d", strtotime($shippingdateforadminfilter));
				/*if($shippingdateforadminfilter1 == "0000-00-00"){
					continue;
				} 
				echo $shippingdateforadminfilter1.'here';
			}
		}*/
		// die;
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "order` SET invoice_prefix = '" . $this->db->escape($data['invoice_prefix']) . "', store_id = '" . (int)$data['store_id'] . "', store_name = '" . $this->db->escape($data['store_name']) . "', store_url = '" . $this->db->escape($data['store_url']) . "', customer_id = '" . (int)$data['customer_id'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : '') . "', payment_firstname = '" . $this->db->escape($data['payment_firstname']) . "', payment_lastname = '" . $this->db->escape($data['payment_lastname']) . "', payment_email= '" . $this->db->escape($data['payment_email']) . "', payment_mobile= '" . $this->db->escape($data['payment_mobile']) . "', payment_company = '" . $this->db->escape($data['payment_company']) . "', payment_address_1 = '" . $this->db->escape($data['payment_address_1']) . "', payment_address_2 = '" . $this->db->escape($data['payment_address_2']) . "', payment_city = '" . $this->db->escape($data['payment_city']) . "', payment_postcode = '" . $this->db->escape($data['payment_postcode']) . "', payment_country = '" . $this->db->escape($data['payment_country']) . "', payment_country_id = '" . (int)$data['payment_country_id'] . "', payment_zone = '" . $this->db->escape($data['payment_zone']) . "', payment_zone_id = '" . (int)$data['payment_zone_id'] . "', payment_address_format = '" . $this->db->escape($data['payment_address_format']) . "', payment_custom_field = '" . $this->db->escape(isset($data['payment_custom_field']) ? json_encode($data['payment_custom_field']) : '') . "', payment_method = '" . $this->db->escape($data['payment_method']) . "', payment_code = '" . $this->db->escape($data['payment_code']) . "', shipping_firstname = '" . $this->db->escape($data['shipping_firstname']) . "', shipping_lastname = '" . $this->db->escape($data['shipping_lastname']) . "', shipping_company = '" . $this->db->escape($data['shipping_company']) . "', shipping_address_1 = '" . $this->db->escape($data['shipping_address_1']) . "', shipping_address_2 = '" . $this->db->escape($data['shipping_address_2']) . "', shipping_phone = '" . $this->db->escape($data['shipping_phone']) . "', shipping_city = '" . $this->db->escape($data['shipping_city']) . "', shipping_postcode = '" . $this->db->escape($data['shipping_postcode']) . "', shipping_country = '" . $this->db->escape($data['shipping_country']) . "', shipping_country_id = '" . (int)$data['shipping_country_id'] . "', shipping_zone = '" . $this->db->escape($data['shipping_zone']) . "', shipping_zone_id = '" . (int)$data['shipping_zone_id'] . "', shipping_address_format = '" . $this->db->escape($data['shipping_address_format']) . "', shipping_custom_field = '" . $this->db->escape(isset($data['shipping_custom_field']) ? json_encode($data['shipping_custom_field']) : '') . "', shipping_method = '" . $this->db->escape($data['shipping_method']) . "', shipping_code = '" . $this->db->escape($data['shipping_code']) . "', comment = '" . $this->db->escape($data['comment']) . "', total = '" . (float)$data['total'] . "', affiliate_id = '" . (int)$data['affiliate_id'] . "', commission = '" . (float)$data['commission'] . "', marketing_id = '" . (int)$data['marketing_id'] . "', tracking = '" . $this->db->escape($data['tracking']) . "', language_id = '" . (int)$data['language_id'] . "', currency_id = '" . (int)$data['currency_id'] . "', currency_code = '" . $this->db->escape($data['currency_code']) . "', currency_value = '" . (float)$data['currency_value'] . "', ip = '" . $this->db->escape($data['ip']) . "', forwarded_ip = '" .  $this->db->escape($data['forwarded_ip']) . "', disclose_sender_details = '" .  $this->db->escape($data['disclose_sender_details']) . "', user_agent = '" . $this->db->escape($data['user_agent']) . "', accept_language = '" . $this->db->escape($data['accept_language']) . "', mr_mrs = '" . $this->db->escape($data['mr_mrs']) . "', phone_code = '" . $this->db->escape($data['phone_code']) . "', payment_mr_mrs = '" . $this->db->escape($data['payment_mr_mrs']) . "', payment_phone_code = '" . $this->db->escape($data['payment_phone_code']) . "', shipping_mr_mrs = '" . $this->db->escape($data['shipping_mr_mrs']) . "', shipping_phone_code = '" . $this->db->escape($data['shipping_phone_code']) . "', date_added = NOW(), date_modified = NOW()");

		$order_id = $this->db->getLastId();
		$this->db->query("UPDATE ".DB_PREFIX."followup set status = 5 WHERE customer_email = '" . $data['email'] . "' AND customer_phone = '" . $data['telephone'] . "'");
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "', shipping_info = '" . $product['shipping_info'] . "', shipping_price = '" . $product['shipping_price'] . "'");
				$order_product_id = $this->db->getLastId();

				if(!empty(json_decode($product['shipping_info'], true))){
					$shippingDate = (array)json_decode($product['shipping_info'], true);
					$shippingdateforadminfilter = $shippingDate['Shipping Date'];
					$shippingdateforadminfiltertime = $shippingDate['Shipping Time'];
					$shippingdateforadminfilter1 = date("Y-m-d", strtotime($shippingdateforadminfilter));
				...
				$this->db->query("UPDATE oc_product SET purchased = purchased+1 WHERE product_id = '".(int)$product['product_id']."'");
			}
		}
		...
		$this->db->query("UPDATE " . DB_PREFIX . "order SET date_forshipping = '" . $date_forshipping . "', slot_forshipping = '".$slot_forshipping."' WHERE order_id = '" . (int)$order_id . "'");

		return $order_id;
	}


 	public function editMobile($cid, $mobile) {
		$mobilChcked = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer` WHERE customer_id = '" .(int)$cid. "'");
		 
		if ($mobilChcked->num_rows) {
			 
			if(empty(trim($mobilChcked->row['telephone']))){
				 
					$this->db->query("UPDATE `" . DB_PREFIX . "customer` SET telephone = '".$this->db->escape($mobile)."' WHERE customer_id = '" .(int)$cid. "'");
			}
		 } 
	} 
	...
	public function addFollowUp($cname,$ctelephone,$cemail,$ttl) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "followup SET customer_name = '" . $cname . "', customer_email = '" . $cemail . "', customer_phone = '" . $ctelephone . "', order_total = '" . $ttl . "', status = 1, session_id = '" . $this->db->escape($this->session->getId()) . "', date_added = NOW()");
	}

	public function addCustomize($cname,$ctelephone,$cemail,$enquiry,$image) {
		$this->db->query("INSERT INTO `oc_followup` (`customer_name`, `customer_email`, `customer_phone`, `status`, `session_id`, `order_total`, `date_added`, `reminder_mail`, `type`, `describe`, `image`) VALUES ('" . $cname . "', '" . $cemail . "', '" . $ctelephone . "', '1', '" . $this->db->escape($this->session->getId()) . "', '0', NOW(), '1', 'customize', '".$enquiry."', '".$image."')");
	}

	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		$order_info = $this->getOrder($order_id);
		$order_status_id_new = $order_status_id;
		if ($order_info) {
			// Fraud Detection
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				$order_status_id = $order_status_id_new;
			...
			if (!$order_info['order_status_id'] && $order_status_id && $order_status_id!=4) {
