...
class ModelAccountOrder extends Model {
	...
	$query = $this->db->query("SELECT  os.name as status, o.* FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id)  WHERE o.customer_id = '" . (int)$this->customer->getId() . "' AND o.order_id='".$order_id."'    ORDER BY o.order_id DESC");
	...
	public function getOrderProductsOnsummary($order_id) {
	...
	public function getTrackOrder($order_id,$mail) {
		$query = $this->db->query("SELECT os.name FROM `" . DB_PREFIX . "order` as o , `" . DB_PREFIX . "order_status` os WHERE  os.order_status_id = o.order_status_id  and o.order_id = '" . (int)$order_id . "' and o.email = '" . $mail . "'");
		if($query->num_rows>0){
			return $query->row['name'];
		} else {
			return $query->row['name']=0;
		}
