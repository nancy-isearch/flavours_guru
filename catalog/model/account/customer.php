...
class ModelAccountCustomer extends Model {
	public function addCustomer($data, $scriptOrder = 0) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', language_id = '" . (int)$this->config->get('config_language_id') . "', firstname = '" . $this->db->escape($data['firstname']) . "', email = '" . trim($this->db->escape($data['email'])) . "', telephone = '" . trim($this->db->escape($data['telephone'])) . "', telephone_code = '" . trim($this->db->escape($data['telephone_code'])) . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "',ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '1', approved = '" . (int)!$customer_group_info['approval'] . "', date_added = NOW(), script_order = '".$scriptOrder."'");
