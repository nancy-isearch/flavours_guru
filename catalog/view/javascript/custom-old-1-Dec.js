jQuery(document).ready(function(){
  ...
  if ($(window).width() >= 769){      // js for arrow downor up
	 ...
	 if ($(window).width() < 768){      // js for arrow downor up for mobile
       ...
       var scroller = $(this).scrollTop();
          if (scroller == 0) {
		   ...
		   $(".topfilterrow").addClass('filter-fixdown');
		   $(".topfilterrow").removeClass('filter-fixup');
		   jQuery(".mob_t-link").removeClass("show");
             } 
		  else if (scroller+senseSpeed < previousScroll) {
	...
	/*jQuery("div#offerBanner span").click(function(){
		jQuery("div#offerBanner").slideUp();
	})*/
	
	
	jQuery('div#offerBanner span').on('click', function() {   
		if(localStorage.showoffers !== '1') {
		   jQuery("div#offerBanner").removeClass("showoffers");
		   localStorage.showoffers='1';
		   jQuery("body").removeClass("offermsg");
		   $('.topfilterrow').removeClass('show');	
...
jQuery(document).ready(function(){
if($('#offerBanner').hasClass('showoffers')){
		$('.topfilterrow').addClass('show');
	  }
  else {
