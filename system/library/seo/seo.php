...
final class Seo {
	...
	public static function findGeneral($registry) {
		...
		$sql = "SELECT DISTINCT id FROM `" . DB_PREFIX . "seo_data` WHERE `type` = 'general'";
		...
		foreach($query->rows as $row) {
			$object_array[] = self::getObject('general', $row['id'], $registry);
		}
	...
	public static function findProducts($registry, $data = array()) {
        ...
        $sql = "SELECT DISTINCT p.product_id FROM `" . DB_PREFIX . "product` p INNER JOIN `".DB_PREFIX."product_description` pd ON (p.`product_id` = pd.`product_id`)  ";
	...
	public static function findProductsTotal($registry, $data = array()) {
        ...
        $sql = "SELECT DISTINCT p.product_id FROM `" . DB_PREFIX . "product` p INNER JOIN `".DB_PREFIX."product_description` pd ON (p.`product_id` = pd.`product_id`) WHERE pd.language_id = '" . (int)$config_language_id . "' ";
	...
	public static function findCategories($registry, $data = array()) {
		...
		$sql = "SELECT DISTINCT c.category_id FROM `" . DB_PREFIX . "category` c INNER JOIN `".DB_PREFIX."category_description` cd ON (c.`category_id` = cd.`category_id`) WHERE cd.language_id = '" . (int)$config_language_id . "' ";
	...
	public static function findManufacturers($registry) {
		...
		$sql = "SELECT DISTINCT manufacturer_id FROM `" . DB_PREFIX . "manufacturer`";
	...
	public static function findInformations($registry) {
		...
		$sql = "SELECT DISTINCT information_id FROM `" . DB_PREFIX . "information`";
	...
	public static function findInformationsTotal($registry) {
		...
		$sql = "SELECT DISTINCT information_id FROM `" . DB_PREFIX . "information_description`";
