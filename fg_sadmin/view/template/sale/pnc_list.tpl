    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <div>
          <div class="well">
            <div class="row">
              <form method="post" action="index.php?route=sale/pnc/downloadcsv&token=<?php echo $token; ?>">
              <div class="col-sm-12">
          ...
          <div class="table-responsive">
            <table class="table table-bordered table-hover">
              ...
              <tbody>
                ...
                <tr>
                  <td class="text-left"><a href="<?php echo $pnc['order_link'] ?>"><?php echo $pnc['id']; ?></a></td>
                  <td class="text-left"><?php echo $pnc['customer_name']; ?></td>
                  <td class="text-left"><?php echo $pnc['customer_email']; ?></td>
                  <td class="text-left"><?php echo $pnc['customer_phone']; ?></td>
                  <td class="text-left"><?php echo round($pnc['order_total']); ?></td>
                  <td class="text-left"><?php echo $pnc['date_added']; ?></td>
                  <td class="text-left"><?php echo $statuses[$pnc['pnc_status']]; ?></td>
                  <td class="text-left"><?php echo ucfirst($adminusers[$pnc['assigned_to']]); ?></td>
                  <td class="text-left">
                    <input type="hidden" class="id_hidden" value="<?php echo $pnc['id']; ?>">
                    <?php if($pnc['assigned_to'] > 0){ ?>
                    ...
                    <button class="btn btn-primary add_btn" type="button">Add</button>
                  <?php } else { ?>
                      <button class="btn btn-primary btn-sm self_assign" type="button">Self Assign</button>
                    <?php } ?>
                  </td>
                  <td class="text-right"><a href="<?php echo $pnc['edit']; ?>" data-toggle="tooltip" title="<?php echo $button_edit; ?>" class="btn btn-primary"><i class="fa fa-pencil"></i></a></td>
                </tr>
                <?php } ?>
                <?php } else { ?>
                <tr>
...
$('#button-filter').on('click', function() {
	var url = 'index.php?route=sale/pnc&token=<?php echo $token; ?>';
...
<script type="text/javascript">
  $('.add_btn').click(function(){
    ...
    $.ajax({
      url: 'index.php?route=sale/pnc/savecomment&token=<?php echo $token; ?>',
      method : 'POST',
  ...
  $('.self_assign').click(function(){
    ...
    $.ajax({
      url: 'index.php?route=sale/pnc/selfassign&token=<?php echo $token; ?>',
      method : 'POST',
      data: {id: id},
      success: function(json) {
