      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-product" class="form-horizontal">
          <ul class="nav nav-tabs">
            ...
            <li><a href="#tab-type" data-toggle="tab"><?php echo "Shipping Type"; ?></a></li>
          </ul>
            ...
            <div class="tab-pane" id="tab-type">
              <div class="table-responsive">
                <table class="table table-bordered table-hover">
                  <tbody>
                    <tr>
                      <td class="text-left">Product Shipping Type</td>
                      <td class="text-left">
                        <select name="product_shipping_type" class="form-control" id="input_product_shippingtype" onchange="enable();">
                        	<option value="">Select Option</option>
                            <option value="0" <?php if( isset($product_addon_shipping_type)){ if($product_addon_shipping_type == "0"){ echo 'selected="selected"';}}?>>Express Shipping</option>
                          	<option value="1" <?php if( isset($product_addon_shipping_type)){ if($product_addon_shipping_type == "1"){ echo 'selected="selected"';}}?>>Courier Shipping</option>
                        </select>
                      </td>
                    </tr>
                    <tr style="display:none;" id="groupcheckedblock">
                      <td class="text-left"><?php echo "Shipping Type"; ?></td>
                      <td class="text-left" >
                        <?php 
                        foreach ($groupschecked as $groupchecked) {
                               $groupchecked1[] = $groupchecked['shipping_id'];
                        }
                    ...
                    <tr>
                      <td class="text-left">Product Add-on</td>
                      <td class="text-left">
                        <select name="product_addon" class="form-control" id="input_product_addon" onchange="showAddon();">
                        	<option>Select Option</option>
                          	<option value="1" <?php if( isset($product_addon_status)){ if($product_addon_status=="1"){ echo 'selected="selected"';}}?>>Yes</option>
                          	<option value="0" <?php if( isset($product_addon_status)){ if($product_addon_status=="0"){ echo 'selected="selected"';}}?>>No</option>
                        </select>
                      </td>
                    </tr>
                    <tr style="display:none;" id="groupaddonblock">
                      <td class="text-left">Product Add-on Categories</td>
                      <td class="text-left col-sm-2" >
                      <select name="product_addon_category" class="form-control" id="input_product_addon_category">
                      <?php 
                        foreach($addoncategories as $addoncategory)
                       { ?>
                        <option value="<?php echo $addoncategory['sub_category_id'] ?>" <?php if( isset($product_addon_category)){ if($product_addon_category == $addoncategory['sub_category_id']){ echo 'selected="selected"';}}?>><?php echo $addoncategory['sub_category_name']; ?></option>
                      <?php }
                      ?>
                        </select>
                      </td>
...
$('input[name=\'category\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/category/autocompletemulti&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
...
function enable() {
  var value = $("#input_product_shippingtype").val();
  if(value == '0')
  {
    $("#groupcheckedblock").show();
  }
  else
  {
   $("#groupcheckedblock").hide(); 
  }
  
}
function showAddon() {
  var value = $("#input_product_addon").val();
  //console.log(value);
  if(value == '1')
  {
    $("#groupaddonblock").show();
  }
  else
  {
   $("#groupaddonblock").hide(); 
  }
  
}

$(document).ready(function(){
  var val1 = <?php echo $product_addon_shipping_type; ?>;
  var val2 = <?php echo $product_addon_status; ?>;
  console.log(val2);
  if(val1 == '0' )
  {
    $("#groupcheckedblock").show();
