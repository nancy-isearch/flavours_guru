...
<?php if ($logged && isset($group_id) && $group_id > 15 && $group_id < 22) { ?>
<audio id="myAudio">
  <source src="view/noti.mp3" type="audio/mpeg">
</audio>
<button onclick="playAudio()" type="button">Play Notification</button>
<script type="text/javascript">
  ...
  function playAudio() { 
    ...
    $.ajax({
      url: 'index.php?route=sale/followup/getNotificationCount&token=<?php echo $token; ?>',
      type: 'post',
      data: 'pins='+1,
      dataType: 'json',
      success: function(data) {
        if(data.followup > 0){
          $.notify("Hey! Got new Followup. Please check", "warn"); 
          x.play();
        }

        if(data.customize > 0){
          $.notify("Hey! Got new Customize. Please check", "warn"); 
          x.play();
        }

        if(data.pnc > 0){
          $.notify("Hey! Got new PNC. Please check", "warn");  
          x.play();
        }
