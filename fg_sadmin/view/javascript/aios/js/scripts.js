...
(function() {
   ...
   function searchform_reposition() {
    ...
    $('#menu a[href]').on('click', function() {
        sessionStorage.setItem('side-navigation', $(this).attr('href'));
    });
  ...
  $('.side-navigation a[href]').on('click', function() {
    ...
    if (!sessionStorage.getItem('side-navigation')) {
      //$('#menu #dashboard').addClass('active');
    } else {
      // Sets active and open to selected page in the left column menu.
      $('.side-navigation a[href=\'' + sessionStorage.getItem('side-navigation') + '\']').parents('li').eq(0).addClass('active');
      $('.side-navigation a[href=\'' + sessionStorage.getItem('side-navigation') + '\']').parents('li').eq(1).addClass('nav-active');
    }

  function shiftMenu() {
      if($('nav#column-left').hasClass("active")) {
        $('.sticky-header').removeClass("sticky-header235");
      } else {
        $('.sticky-header').addClass("sticky-header235");
      }
   }
