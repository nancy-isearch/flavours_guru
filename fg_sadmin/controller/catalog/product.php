...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		$data['tab_type'] = $this->language->get('tab_design');
		...
		if (isset($this->request->post['product_addon_shipping_type'])) {
			$data['product_addon_shipping_type'] = $this->request->post['product_addon_shipping_type'];
		} elseif (!empty($product_info)) {
			$data['product_addon_shipping_type'] = $product_info['product_addon_shipping_type'];
		} else {
			$data['product_addon_shipping_type'] = '';
		}

		if (isset($this->request->post['product_addon_status'])) {
			$data['product_addon_status'] = $this->request->post['product_addon_status'];
		} elseif (!empty($product_info)) {
			$data['product_addon_status'] = $product_info['product_addon_status'];
		} else {
			$data['product_addon_status'] = '';
		}

		if (isset($this->request->post['product_addon_category'])) {
			$data['product_addon_category'] = $this->request->post['product_addon_category'];
		} elseif (!empty($product_info)) {
			$data['product_addon_category'] = $product_info['product_addon_category'];
		} else {
			$data['product_addon_category'] = '';
		}
		...
		foreach ($categories as $category_id) {
			$category_info = $this->model_catalog_category->getMultiCategory($category_id);

			if ($category_info) {
				$data['product_categories'][] = array(
					'category_id' => $category_info['id'],
					'name'        => $category_info['cpname']!=NULL?$category_info['cpname']:$category_info['name']
				);
			}
		}

		$data['addoncategories']=$this->model_catalog_product->getAddonCategories();
		...
		if (isset($this->request->post['citygp'])) {
			$data['groupschecked'] = $this->request->post['citygp'];
		} elseif (isset($this->request->get['product_id'])) {
			$data['groupschecked'] = $this->model_catalog_product->getcheckedgroups($this->request->get['product_id']);
		} else {
			$data['groupschecked'] = array();
		}

		$this->load->model('shipping/type');
		$data['types'] = $this->model_shipping_type->getShippingslot();
