...
class ControllerSaleComplaint extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_sale_complaint->addCities($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_sale_complaint->editFollowup($this->request->get['id'], $this->request->post);
			...
			$this->response->redirect($this->url->link('sale/complaint', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $city_id) {
				$this->model_sale_complaint->deletecities($city_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/complaint', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('sale/complaint/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['copy'] = $this->url->link('sale/complaint/copy', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('sale/complaint/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		$cities_total = $this->model_sale_complaint->getTotalFollowups($filter_data);

		$results = $this->model_sale_complaint->getFollowups($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['followups'][] = array(
				...
				'edit'       => $this->url->link('sale/complaint/edit', 'token=' . $this->session->data['token'] . '&id=' . $result['id'] . $url, true)
			);
		}
		...
		$data['exportcsv'] = $this->url->link('sale/complaint/exportcsv', 'token=' . $this->session->data['token'], true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['id'])) {
			$data['action'] = $this->url->link('sale/complaint/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('sale/complaint/edit', 'token=' . $this->session->data['token'] . '&id=' . $this->request->get['id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('sale/complaint', 'token=' . $this->session->data['token'] . $url, true);

		if (isset($this->request->get['id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$data['followup_info'] = $this->model_sale_complaint->getFollowup($this->request->get['id']);
			$data['followup_comments'] = $this->model_sale_complaint->getFollowupComments($this->request->get['id']);
		}
		...
		//echo "<pre />"; print_r($data); die();
		$this->response->setOutput($this->load->view('sale/complaint_form', $data));
	}
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_sale_complaint->getCities($filter_data);
			foreach ($results as $result) {
	...
	public function savecomment() {
		$this->load->model('sale/complaint');
		$this->model_sale_complaint->editComplaint($_POST['id'], $_POST);
	}

	public function downloadcsv(){
		$this->load->model('sale/complaint');
		$filter_data = $this->request->post;
		$results = $this->model_sale_complaint->csvdata($filter_data);
		$adminusers = $this->model_sale_complaint->adminusers();
