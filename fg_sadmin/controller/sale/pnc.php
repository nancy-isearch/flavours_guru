<?php
class ControllerSalePnc extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('sale/pnc');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('sale/pnc');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_sale_pnc->addCities($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_sale_pnc->editPnc($this->request->get['id'], $this->request->post);
			...
			$this->response->redirect($this->url->link('sale/pnc', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $city_id) {
				$this->model_sale_pnc->deletecities($city_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/pnc', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('sale/pnc/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['copy'] = $this->url->link('sale/pnc/copy', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('sale/pnc/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		$cities_total = $this->model_sale_pnc->getTotalPnc($filter_data);

		$results = $this->model_sale_pnc->getPncs($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['pncs'][] = array(
				...
				'pnc_status'	=> $result['pnc_status'],
				'date_added'	=> $result['date_added'],
				'assigned_to'	=> $result['assigned_to'],
				'edit'       => $this->url->link('sale/pnc/edit', 'token=' . $this->session->data['token'] . '&id=' . $result['order_id'] . $url, true)
			);
		}
		...
		$data['exportcsv'] = $this->url->link('sale/pnc/exportcsv', 'token=' . $this->session->data['token'], true);
		...
		$data['adminusers'] = $this->model_sale_pnc->adminusers();
		$this->response->setOutput($this->load->view('sale/pnc_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['id'])) {
			$data['action'] = $this->url->link('sale/pnc/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('sale/pnc/edit', 'token=' . $this->session->data['token'] . '&id=' . $this->request->get['id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('sale/pnc', 'token=' . $this->session->data['token'] . $url, true);

		if (isset($this->request->get['id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$data['pnc_info'] = $this->model_sale_pnc->getPnc($this->request->get['id']);
			$data['pnc_comments'] = $this->model_sale_pnc->getPncComments($this->request->get['id']);
			$data['adminusers'] = $this->model_sale_pnc->adminusers();
		}
		...
		$data['statuses'] = $this->getStatus();
		//echo "<pre />"; print_r($data); die();
		$this->response->setOutput($this->load->view('sale/pnc_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'sale/pnc')) {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_sale_pnc->getCities($filter_data);
			foreach ($results as $result) {
	...
	public function savecomment() {
		$this->load->model('sale/pnc');
		$this->model_sale_pnc->editPnc($_POST['id'], $_POST);
	}

	public function selfAssign(){
		$this->load->model('sale/pnc');
		$this->model_sale_pnc->selfAssign($_POST['id']);	
	}

	public function downloadcsv(){
		$this->load->model('sale/pnc');
		$filter_data = $this->request->post;
		$results = $this->model_sale_pnc->csvdata($filter_data);
		$adminusers = $this->model_sale_pnc->adminusers();
