...
class ControllerSaleFollowup extends Controller {
	...
	public function index() {
		$this->load->language('sale/followup');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('sale/followup');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_sale_followup->addCities($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_sale_followup->editFollowup($this->request->get['id'], $this->request->post);
			...
			$this->response->redirect($this->url->link('sale/followup', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $city_id) {
				$this->model_sale_followup->deletecities($city_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/followup', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('sale/followup/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['copy'] = $this->url->link('sale/followup/copy', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('sale/followup/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		$cities_total = $this->model_sale_followup->getTotalFollowups($filter_data);

		$results = $this->model_sale_followup->getFollowups($filter_data);

		foreach ($results as $result) {
			$pro = $this->model_sale_followup->getProductsFromSession($result['session_id']);
			$data['followups'][] = array(
				...
				'pro' => $pro,
				'edit'       => $this->url->link('sale/followup/edit', 'token=' . $this->session->data['token'] . '&id=' . $result['id'] . $url, true)
			);
		}
		...
		$data['exportcsv'] = $this->url->link('sale/followup/exportcsv', 'token=' . $this->session->data['token'], true);
		...
		$data['adminusers'] = $this->model_sale_followup->adminusers();
		//echo "<pre />"; print_r($data); die();
		$this->response->setOutput($this->load->view('sale/followup_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['id'])) {
			$data['action'] = $this->url->link('sale/followup/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('sale/followup/edit', 'token=' . $this->session->data['token'] . '&id=' . $this->request->get['id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('sale/followup', 'token=' . $this->session->data['token'] . $url, true);

		if (isset($this->request->get['id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$data['followup_info'] = $this->model_sale_followup->getFollowup($this->request->get['id']);
			$data['followup_comments'] = $this->model_sale_followup->getFollowupComments($this->request->get['id']);
			$data['adminusers'] = $this->model_sale_followup->adminusers();
		}
		...
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
		//$data['statuses'] = array('1' => 'New Lead', '2' => 'Not Interested', '3' => 'Number Busy', '4' => 'Call Back', '5' => 'Order Placed', '6' => 'Not Answering', '7' => 'Closed', '8' => 'Duplicate entry', '9' => 'Connected on Wati', '10' => 'Wrong Number');
		$data['statuses'] = $this->getStatus();
		//echo "<pre />"; print_r($data); die();
		$this->response->setOutput($this->load->view('sale/followup_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'sale/followup')) {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_sale_followup->getCities($filter_data);
			foreach ($results as $result) {
	...
	public function savecomment() {
		$this->load->model('sale/followup');
		$this->model_sale_followup->editFollowup($_POST['id'], $_POST);
	}

	public function selfAssign(){
		$this->load->model('sale/followup');
		$this->model_sale_followup->selfAssign($_POST['id']);	
	}

	public function getNotificationCount(){
		$this->load->model('sale/followup');
		$data = $this->model_sale_followup->getNotificationCount();
	...
	public function downloadcsv(){
		...
		$results = $this->model_sale_followup->csvdata($filter_data);
		$adminusers = $this->model_sale_followup->adminusers();
