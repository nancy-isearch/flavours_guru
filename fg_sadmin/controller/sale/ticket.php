<?php
class ControllerSaleTicket extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('sale/ticket');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_sale_ticket->addCities($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_sale_ticket->editFollowup($this->request->get['id'], $this->request->post);
			...
			$this->response->redirect($this->url->link('sale/ticket', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $city_id) {
				$this->model_sale_ticket->deletecities($city_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/ticket', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('sale/ticket/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['copy'] = $this->url->link('sale/ticket/copy', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('sale/ticket/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		$cities_total = $this->model_sale_ticket->getTotalTickets($filter_data);

		$results = $this->model_sale_ticket->getTickets($filter_data);
		//echo "<pre />"; print_r($results); die();
		foreach ($results as $result) {

			$data['tickets'][] = array(
				'id' => $result['cid'],
				'ccreated_at'      => $result['ccreated_at'],
				...
				'status'	=> $result['cstatus'],
				'comments' => $result['comments'],
				'edit'       => $this->url->link('sale/ticket/edit', 'token=' . $this->session->data['token'] . '&id=' . $result['id'] . $url, true)
			);
		}
		...
		$data['exportcsv'] = $this->url->link('sale/ticket/exportcsv', 'token=' . $this->session->data['token'], true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['id'])) {
			$data['action'] = $this->url->link('sale/ticket/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('sale/ticket/edit', 'token=' . $this->session->data['token'] . '&id=' . $this->request->get['id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('sale/ticket', 'token=' . $this->session->data['token'] . $url, true);

		if (isset($this->request->get['id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$data['followup_info'] = $this->model_sale_ticket->getFollowup($this->request->get['id']);
			$data['followup_comments'] = $this->model_sale_ticket->getFollowupComments($this->request->get['id']);
		}
		...
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
		$data['statuses'] = array('1' => 'New Lead', '2' => 'Not Interested', '3' => 'Number Busy', '4' => 'Call Back', '5' => 'Order Placed', '6' => 'Not Answering', '7' => 'Closed');
		//echo "<pre />"; print_r($data); die();
		$this->response->setOutput($this->load->view('sale/ticket_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'sale/ticket')) {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_sale_ticket->getCities($filter_data);
			foreach ($results as $result) {
	...
	public function savecomment() {
		$this->load->model('sale/ticket');
		$this->model_sale_ticket->editTicket($_POST['id'], $_POST);
	}

	public function downloadcsv(){
		$this->load->model('sale/ticket');
		$filter_data = $this->request->post;
		$results = $this->model_sale_ticket->csvdata($filter_data);
		$adminusers = $this->model_sale_ticket->adminusers();
		header('Content-Type: text/csv; charset=utf-8');
		header('Content-Disposition: attachment; filename=complaint-'.date("d-m-Y").'.csv');
		$output = fopen('php://output', 'w');
		fputcsv($output, array('Date', 'City', 'Order No.', 'Complaint Type', 'Resolution', 'Vendor', 'Fault', 'Status'));
		...
		foreach ($results as $rslt) {
			$aa = array();
			$aa[] = date('d/m/Y', strtotime($rslt['ccreated_at']));
			$aa[] = $rslt['shipping_city'];
			$aa[] = $rslt['order_id'];
			$aa[] = $complaintstype[$rslt['complaint_type']];
			$aa[] = $resolutions[$rslt['resolution']];
			$aa[] = ucfirst($adminusers[$rslt['vendor_id']]);
			$aa[] = $faults[$rslt['fault']];
			$aa[] = $statuses[$rslt['status']];
			$all[] = $aa;
		}
		if (count($all) > 0) {
		    foreach ($all as $al) {
