<?php
class ControllerReportSaleOrderDetailVendor extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Order Detail',
			'href' => $this->url->link('report/sale_order_detail_vendor', 'token=' . $this->session->data['token'] . $url, true)
		);
		...
		$order_total = $this->model_report_sale->getTotalOrdersDetailn($filter_data);

		$results = $this->model_report_sale->getOrdersDetailn($filter_data);
		...
		$pagination->url = $this->url->link('report/sale_order_detail_vendor', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('report/sale_order_detail_vendor', $data));
	}

	public function downloadcsv(){
		$filter_data = $this->request->post;
		$this->load->model('report/sale');
		$results = $this->model_report_sale->getOrdersDetailVendor($filter_data);
		$orders = array();
		foreach ($results as $result) {
			$orders[] = array(
	...
	public function downloaddeliveredordercsv(){
		$filter_data = $this->request->post;
		$this->load->model('report/sale');
		$results = $this->model_report_sale->getOrdersDetailDeliveredVendor($filter_data);
		$orders = array();
		foreach ($results as $result) {
			$orders[] = array(
				'dateadded' => $result['dateadded'],
		...
		header('Content-Disposition: attachment; filename=order_details_'.date("d-m-Y").'.csv');
		$output = fopen('php://output', 'w');
		fputcsv($output, array('Date Added', 'Time Added', 'Order No.', 'Customer Name', 'Contact Number', 'Delivery Date', 'Timeslot', 'Shipping Type', 'Delivery Pin', 'Delivery City', 'Payment Mode', 'Transaction ID', 'Product Name', 'Weight', 'SKU', 'Quantity', /*'Unit Price', 'GST', 'Total Amount After GST', 'Total Order Amount', */ 'PP', 'Order Total PP', 'Status', 'Vendor', 'Delivered Date', 'Delivered Time', 'Added by Backend?', 'Added By', 'Delivery Boy', 'Delivered By', 'Delivered At'));
		if (count($orders) > 0) {
		    foreach ($orders as $al) {
		        fputcsv($output, $al);
		    }
