...
class ModelSaleOdb extends Model {
	...
	public function getOrdersForOdb(){
		...
		if($tmp){
			...
			if (!empty($data['status'])) {
				...
				//$sql .= " AND (od.id is null OR od.status >= 0 )";
			}
		}
	...
	public function csvdata($data){
		...
		$sql = "SELECT f.order_id mainid, payment_firstname, payment_mobile, payment_city, payment_email, shipping_firstname, shipping_phone, shipping_city, shipping_email, f.date_added fdate_added, f.order_status_id fstatus, date_forshipping, is_admin_order, total FROM oc_order f where f.order_status_id >= 0 ";
