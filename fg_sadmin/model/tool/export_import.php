...
class ModelToolExportImport extends Model {
	...
	protected function storeCategoryIntoDatabase( &$category, &$languages, $exist_meta_title, &$layout_ids, &$available_store_ids, &$url_alias_ids ) {
		...
		if ($seo_keyword) {
			if (isset($url_alias_ids[$category_id])) {
				...
				$sql = "INSERT INTO `".DB_PREFIX."url_alias` (`url_alias_id`,`query`,`keyword`,`lang`) VALUES ($url_alias_id,'category_id=$category_id','$seo_keyword','1');";
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		$product_addon_shipping_type = $product['product_addon_shipping_type'];
		$product_addon_status = $product['product_addon_status'];
		$product_addon_category = $product['product_addon_category'];
		$product_to_shipping = $product['product_to_shipping'];
		...
		$sql .= "`tax_class_id`,`viewed`,`length`,`width`,`height`,`length_class_id`,`sort_order`,`subtract`,`minimum`,`product_addon_shipping_type`,`product_addon_status`,`product_addon_category`) VALUES ";
		...
		$sql .= "$tax_class_id,$viewed,$length,$width,$height,'$length_class_id','$sort_order','$subtract','$minimum','$product_addon_shipping_type','$product_addon_status','$product_addon_category');";
		$this->db->query($sql);
		foreach ($languages as $language) {
			$language_code = $language['code'];
			$language_id = $language['language_id'];
		...
		if($product_addon_shipping_type > 0){
			$this->db->query("DELETE FROM `oc_shipping_to_product` WHERE `product_id` = '".$product_id."'");
		}
		if (count($product_to_shipping) > 0 && $product_addon_shipping_type == 0) {
			$this->db->query("DELETE FROM `oc_shipping_to_product` WHERE `product_id` = '".$product_id."'");
			$sql = "INSERT INTO `".DB_PREFIX."shipping_to_product` (`product_id`,`shipping_id`) VALUES ";
			$first = true;
			foreach ($product_to_shipping as $shipping_id) {
				$sql .= ($first) ? "\n" : ",\n";
				$first = false;
				$sql .= "($product_id,$shipping_id)";
			}
			$sql .= ";";
			$this->db->query($sql);
		}
		...
		if ($keyword) {
			if (isset($url_alias_ids[$product_id])) {
				...
				$sql = "INSERT INTO `".DB_PREFIX."url_alias` (`url_alias_id`,`query`,`keyword`,`lang`) VALUES ($url_alias_id,'product_id=$product_id','$keyword','1');";
	...
	protected function deleteProducts( $exist_table_product_tag, &$url_alias_ids ) {
		...
		$sql .= "TRUNCATE TABLE `".DB_PREFIX."shipping_to_product`;\n";
	...
	protected function deleteProduct( $product_id, $exist_table_product_tag ) {
		...
		$sql .= "DELETE FROM `".DB_PREFIX."shipping_to_product` WHERE `product_id` = '$product_id';\n";
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$product_addon_shipping_type = $this->getCell($data,$i,$j++,'1');
			$product_addon_status = $this->getCell($data,$i,$j++,'1');
			$product_addon_category = $this->getCell($data,$i,$j++,'1');
			$product_to_shipping = $this->getCell($data,$i,$j++,'1');
			$product = array();
			...
			$product_to_shipping = trim( $this->clean($product_to_shipping, false) );
			$product['product_to_shipping'] = ($product_to_shipping=="") ? array() : explode( ",", $product_to_shipping );
			if ($product['product_to_shipping']===false) {
				$product['product_to_shipping'] = array();
			}
			...
			$product['product_addon_shipping_type'] = $product_addon_shipping_type;
			$product['product_addon_status'] = $product_addon_status;
			$product['product_addon_category'] = $product_addon_category;
...
//		}
	...
	protected function getProducts( &$languages, $default_language_id, $product_fields, $exist_meta_title, $offset=null, $rows=null, $min_id=null, $max_id=null ) {
		...
		$sql .= "  GROUP_CONCAT( DISTINCT CAST(psc.shipping_id AS CHAR(11)) SEPARATOR \",\" ) AS shippings,";
		$sql .= "  p.sku,";
		$sql .= "  p.upc,";
		if (in_array( 'ean', $product_fields )) {
			$sql .= "  p.ean,";
		...
		$sql .= "  p.subtract, ";
		$sql .= "  p.minimum, ";
		$sql .= "  p.product_addon_shipping_type, ";
		$sql .= "  p.product_addon_status, ";
		$sql .= "  p.product_addon_category, ";
		...
		$sql .= "LEFT JOIN `".DB_PREFIX."shipping_to_product` psc ON p.product_id=psc.product_id ";
	...
	protected function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
		...
		$data[$j++] = 'subtract';
		$data[$j++] = 'minimum';
		$data[$j++] = 'product_addon_shipping_type';
		$data[$j++] = 'product_addon_status';
		$data[$j++] = 'product_addon_category';
		$data[$j++] = 'shipping_to_product';
		$worksheet->getRowDimension($i)->setRowHeight(30);
		$this->setCellRow( $worksheet, $i, $data, $box_format );
		...
		foreach ($products as $row) {
			...
			$data[$j++] = $row['minimum'];
			$data[$j++] = $row['product_addon_shipping_type'];
			$data[$j++] = $row['product_addon_status'];
			$data[$j++] = $row['product_addon_category'];
			$data[$j++] = $row['shippings'];
			$this->setCellRow( $worksheet, $i, $data, $this->null_array, $styles );
			$i += 1;
			$j = 0;
		}
