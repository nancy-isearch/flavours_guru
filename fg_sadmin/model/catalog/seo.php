...
class ModelCatalogSeo extends Model {
					...
					foreach ($products as $product) {
						...
						if($value->num_rows) {
							...
							if(str_replace(".","",VERSION) < 2200) {
								...
								$price = $this->currency->format($product['price'],$this->config->get('config_currency'),'', false);
							}
	...
	private function getProductsForUrl($lid) {
		$query = $this->db->query("SELECT DISTINCT p.product_id, pd.name,pd.meta_title, pd.language_id, p.model, p.price, p.image, m.name as manufacturer_name FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON(p.product_id=p2s.product_id) WHERE pd.language_id = '".(int)$lid."' ORDER BY pd.name ASC");
		return $query->rows;
	}
				    ...
				    if($value->num_rows) {			
						if($value->row['meta_title']) {
						    ...
						    if(str_replace(".","",VERSION) < 2200) {
								...
								$price = $this->currency->format($product['price'],$this->config->get('config_currency'),'', true);
							}
						    $tags = array(
	...
	private function getCategoriesForUrl($lid) {
		$query = $this->db->query("SELECT DISTINCT c.category_id, c.image, cd.name, cd.description, cd.meta_title, cd.language_id FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id=c2s.category_id) WHERE cd.language_id = '".(int)$lid."' ORDER BY c.sort_order, cd.name ASC");
		return $query->rows;
	}
	...
	public function generateManufacturerUrlKeyword($template, $pattern = array()) {
		...
		foreach ($languages as $key => $value1) {
			...
			foreach ($manufacturers as $manufacturer) {
				$value = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE query = 'manufacturer_id=".$manufacturer['manufacturer_id']."' AND lang  = '".$value1['language_id']."'");
				if($value->num_rows) {
					$duplicate = false;
					$slug = $uniqueSlug = $value->row['keyword'];
					$index = 1;
					...
					$slugs[] = $uniqueSlug;
					if($duplicate)
					$this->setUrlAlias('manufacturer_id', $manufacturer['manufacturer_id'], $uniqueSlug, $value1['language_id']);
				} else {
	...
	public function createTablesInDatabse() {
	    ...
	    $url_alias_query = $this->db->query("SELECT * FROM `". DB_PREFIX ."url_alias` WHERE lang = 0");
	    foreach ($url_alias_query->rows as $key => $value) {
	    	$querycount = $this->db->query("SELECT * FROM `". DB_PREFIX ."url_alias` WHERE query = '".$value['query']."'");
	    	if($querycount->num_rows == 1) {
	    		$this->db->query("UPDATE `" . DB_PREFIX . "url_alias` SET lang = '".$this->config->get("config_language_id")."' WHERE query = '".$value['query']."'");
	    	} else {
	    		$this->db->query("DELETE FROM `" . DB_PREFIX . "url_alias` WHERE  query = '".$value['query']."'");
	    	}
	    }
