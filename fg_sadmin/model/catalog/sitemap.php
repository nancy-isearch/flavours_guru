...
class ModelCatalogSitemap extends Model {	
	...
	public function getProducts($server) {
		...
		$prdct_date = array("4","15","812","814","810","811","815","816","817","818","819","820","7","8","9","4149","4429","4426","4425","4427","4413","4398","4410","4395","4147","4686");
		foreach ($results->rows as $key => $value) {
			if (in_array($value['product_id'], $prdct_date)) {
				continue;
				}
	...
	protected function getManufacturers($server) {
		...
		$manuf123 = array("11","12");
		foreach ($this->model_catalog_manufacturer->getManufacturers() as $result) {
			if (in_array($result['manufacturer_id'], $manuf123)) {
				continue;
				}
			$url =  $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE query = 'manufacturer_id=".$result['manufacturer_id']."'");
			if($url->num_rows) {
				$output .= $this->generateLinkMANNode($server. $url->row['keyword'], "yearly", "0.9");
			} else {
				$output .= $this->generateLinkMANNode($server .  'index.php?route=product/manufacturer/info&manufacturer_id=' . $result['manufacturer_id'], "yearly", "0.9");
			}
		}
		return $output;
	}
	...
	protected function getMManufacturers($server) {
		...
		foreach ($this->model_catalog_manufacturer->getManufacturers() as $result) {
			...
			$lang = $this->getLanguages();
			foreach ($lang as $tp => $langdetails) {
				$url =  $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE query = 'manufacturer_id=".$result['manufacturer_id']."' AND lang = '".(int)$langdetails['language_id']."'");
				if($url->num_rows) {
					$keywordvalue = $url->row['keyword'];
					$link = $server . $keywordvalue;
			...
			$output .= $this->generateLinkINode($links,"yearly", "0.9");
		}
		return $output;
	}
	...
	protected function getGeneral($server) {
		$output = '';
		$generalurls =  $this->db->query("SELECT DISTINCT query, keyword FROM " . DB_PREFIX . "url_alias WHERE query LIKE 'route=%' AND lang = '".$this->config->get("config_language_id")."'");
		$people = array("helper","compare","manufacturer","special","advancedlogingoogle","login","affliate-forgotten","affliate-login","affliate-edit","edit","checkout-success","cart","affliate-tracking","failure","not_found","category28-kamla","index","advancedlogin","forgotten","affliate-success","affliate-register","affliate-logout","reset","search","logout","category_content","common_view","cron");
		foreach ($generalurls->rows as $key => $value) {
				if (in_array($value['keyword'], $people)) {
				continue;
				}
			$output .= $this->generateLinkMANNode($server. $value['keyword'], "yearly", "1");
	...
	protected function getMGeneral($server) {
		$output = '';
		$generalurls =  $this->db->query("SELECT DISTINCT query FROM " . DB_PREFIX . "url_alias WHERE query LIKE 'route=%'");

		foreach ($generalurls->rows as $key => $value) {
			$links = array();
			$lang = $this->getLanguages();
			foreach ($lang as $tp => $langdetails) {
				$url =  $this->db->query("SELECT keyword,query FROM " . DB_PREFIX . "url_alias WHERE query LIKE '".$value['query']."' AND lang = '".$langdetails['language_id']."'");
				if($url->num_rows) {
					$keywordvalue = $url->row['keyword'];
					$link = $server . $keywordvalue;
					$links[$value['query']][] = array('link' => $link,'code' => $langdetails['code']);
				} else {
					$link = $server.'/index.php?'.$value['query'];
					$links[$value['query']][] = array('link' => $link,'code' => $langdetails['code']);
				}
			}
			$output .= $this->generateLinkINode($links,"yearly", "1");
		}
		return $output;
	}
